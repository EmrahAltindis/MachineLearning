...
x = pd.DataFrame.to_numpy(x) # NumPy matrisine dönüştürelim.
y = pd.DataFrame.to_numpy(y) # NumPy matrisine dönüştürelim.
...
for i in range(int(len(y))):
    errorSqrtlinear =+ (y[i]- (m*x[i]+b))**2

print("Linear Error = ",errorSqrtlinear)